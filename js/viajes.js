  ...
  getMapaDinamicoMapox(){
    ...
    readInputFile(files){
      ...
      if(!archivo.type.match(tipoXml)){
        ...
        $('main> section:nth-child(3)').append(contenido);

      }else{
        var nombre = $('<h4>').text("Nombre del archivo: " + archivo.name);
        $('main> section:nth-child(3)').append(nombre);
        ...
        lista.append(tipo); 
        lista.append(ultima);
        $('main> section:nth-child(3)').append(lista);
        ...
        lector.onload = function (evento) {
            ...
            for(let i =0;i<rutas.length;i++){
              ...
              info_ruta+=` </p></section>`;
              $('main> section:nth-child(3)').append(info_ruta);
  ...
  async leerArchivo(file) {
    return new Promise((resolve, reject) => {
      ...
      $('main > section:nth-child(4)').append(nombre);
  ...
  async readInputFileKml(files) {
    ...
    var map = new mapboxgl.Map({ 
      container: document.querySelector('main > section:nth-child(4)'), 
      style: 'mapbox://styles/mapbox/streets-v9',  
  ...
  async readInputFileSvg(files){
    for (const file of files) {
      const svgs = await this.leerKml(file);
      for (const svg of svgs) {
        $('section:last').append(svg);
      }
    }
}
